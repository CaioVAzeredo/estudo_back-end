...
const https = require('https') //Adicionado o https
...
server.use(/^(?!\/(public|livros|autores|categorias)).*$/, (req, res, next) => {
 ...
 //Transformando JSON em HTML 
server.get('/public/docs', (req, res) => { //servidor receber o metodo GET no caminho /public/docs, executa o codigo a seguir: 
  const meuHtml = `
    ...
    `
    res.status(200).contentType("text/html").send(meuHtml) //res = resposta, a resposta está setada no status 200(para ver se está tudo certo na comunicação), .contentType()é para setar o conteudo como texto em  vez de JSON. send() é para mandar a variável como resposta 
})

server.use(router)

//linhas de código adicionados por mim para configurar o protocolo https
https.createServer(
  {
  ...
  server
).listen(8000, () => { // escuta da porta 8000 dos arquivos passados por server.key e server.crt que sao os protocolos de segurança https
